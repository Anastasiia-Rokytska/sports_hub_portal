...
public class NotificationService {
    ...
    public void notifySubscribers(Article article){
        ...
        if (team != null) {
            Set<User> subscribers = teamService.getTeamWithSubscribers(article.getTeam()).getSubscribers();
            subscribers.forEach(subscriber -> {
                simpMessagingTemplate.convertAndSendToUser(subscriber.getEmail(), "/team/new-article", article);
            });
        }
    }
}
